...
def project_point_cloud(velodyne, img_msg, box_data, image_pub, points_pub, regions_pub):
    ...
    try:
        transform = TF_BUFFER.lookup_transform('camera2', 'velodyne', rospy.Time(0))
    ...
    inrange = np.where((points3D[:, 2] > -0.3) &
                       (points3D[:, 2] < 20) &
                       (points3D[:, 0] < 5) & (points3D[:,0]>-5)&
                       (points3D[:, 1] < 1) & (points3D[:,1]>-2))
    ...
    box_3D_F = ros_numpy.point_cloud2.array_to_pointcloud2(box_3D_F, frame_id = "camera2",stamp = rospy.Time(0))
    
    points_pub.publish(box_3D_F)
    
    # box_regions
    boxregionarray.header.frame_id = "velodyne"
    boxregionarray.header.stamp = box_3D_F.header.stamp
...
def select_points_in_box(xy_arr, box_info):
    ...
    for i in range(len(box_info)):
        ...
        classlist = ["neveruse","empty1","cone_B","cone_Y","A1","A2","A3","B1","B2","B3"]
        if box_info[i][4] in classlist :
            whatinbox = classlist.index(box_info[i][4])
...
def listener(camera_info, image_color, velodyne_points,yolo_box,camera_lidar=None):
    # Start node
    rospy.init_node('calibrate_camera_lidar2', anonymous=True)
    ...
    if camera_lidar: 
        ...
        points_pub = rospy.Publisher('before_adaptive2',PointCloud2,queue_size = 5)
        regions_pub = rospy.Publisher('box_regions2', BoxregionArray, queue_size = 5)
...
if __name__ == '__main__':
    ...
    camera_info = rospy.get_param('camera_info_topic2')
    image_color = rospy.get_param('image_color_topic2')
    velodyne_points = rospy.get_param('velodyne_points_topic')
    camera_lidar = rospy.get_param('camera_lidar_topic2')
    yolo_box = rospy.get_param('yolo_box_topic2')
