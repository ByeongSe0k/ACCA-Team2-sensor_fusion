...
def project_point_cloud(velodyne, img_msg, box_data, image_pub, points_pub, regions_pub):
    ...
    try:
        transform = TF_BUFFER.lookup_transform('camera1', 'velodyne', rospy.Time(0))
    ...
    box_3D_F = ros_numpy.point_cloud2.array_to_pointcloud2(box_3D_F, frame_id = 'camera1',stamp = rospy.Time(0))
    points_pub.publish(box_3D_F)
    
    # box_regions
    boxregionarray.header.stamp = rospy.Time(0)
    boxregionarray.header.frame_id = 'camera1'
...
def listener(camera_info, image_color, velodyne_points,yolo_box,camera_lidar=None):
    # Start node
    rospy.init_node('calibrate_camera_lidar1', anonymous=True)
