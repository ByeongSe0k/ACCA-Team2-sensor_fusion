...
def project_point_cloud(velodyne, img_msg, box_data, image_pub, points_pub, regions_pub):
    ...
    try:
        transform = TF_BUFFER.lookup_transform('camera', 'velodyne', rospy.Time())
    ...
    if len(points3D) > 0 :
        ...
        box_3D_F = ros_numpy.point_cloud2.array_to_pointcloud2(box_3D_F, frame_id = 'camera',stamp = rospy.Time())
        points_pub.publish(box_3D_F)
        
        # box_regions
        boxregionarray.header.stamp = rospy.Time()
        boxregionarray.header.frame_id = 'camera'
    ...
    #     cv2.circle(img, (int(image_dots[i][0]), int(image_dots[i][1])), 2, tuple(colors[i]), -1)
...
def callback(image, camera_info, velodyne, yolo, image_pub=None, points_pub = None, regions_pub = None):
    ...
    box_data_matching = rospy.get_param('box_data_matching',True)
    box_data = []
    classlist = ["neveruse","empty1","cone_B","cone_Y","A1","A2","A3","B1","B2","B3"]
    
    if box_data_matching :
        sizechange = [1624.0/608,1234.0/608]
        # print(sizechange)
    else :
        sizechange = 1
    for box in yolo.bounding_boxes :
        if box.Class in classlist:
            temp = [box.xmin,box.ymin,box.xmax,box.ymax,box.Class]
            for i in range(4):
                temp[i] *= sizechange[i%2]
            # print(temp)
...
def listener(camera_info, image_color, velodyne_points,yolo_box,camera_lidar=None):
    ...
    if camera_lidar: 
        ...
        points_pub = rospy.Publisher('before_adaptive1',PointCloud2,queue_size = 5)
        regions_pub = rospy.Publisher('box_regions1', BoxregionArray, queue_size = 5)
