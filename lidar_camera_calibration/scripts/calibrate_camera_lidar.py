...
def extract_points_3D(velodyne, now):
    ...
    points = np.asarray(points.tolist()) #if original value changed -> value in asarray changes
    ...
    inrange = np.where((points[:, 0] > 0) &
                       ...
                       (points[:, 1] < 8) &
                       (points[:, 2] < 5))
    points = points[inrange[0]]     #first index in inrange[], (A,B) coordinate shape
    print(points.shape)
    if points.shape[0] > 5:
    ...
    ax.scatter(points[:, 0], points[:, 1], points[:, 2], c=points[:,4], s=2, picker=5)
...
def calibrate(points2D=None, points3D=None):
    ...
    error = (points2D_reproj - points2D)  # Compute error only over inliers.
...
def callback(image, camera_info, velodyne, image_pub=None):
    global CAMERA_MODEL, FIRST_TIME, PAUSE, TF_BUFFER, TF_LISTENER, pub_pt
...
def listener(camera_info, image_color, velodyne_points, camera_lidar=None):
    ...
    pub_pt = rospy.Publisher("pointcloud_calibration",PointCloud2,queue_size=1)
    ...
    ats = message_filters.ApproximateTimeSynchronizer(
        [image_sub, info_sub, velodyne_sub], queue_size=5, slop= 1.7e9)
